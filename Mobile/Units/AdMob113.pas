...
{$IFDEF ANDROID}
  ...
  public
    ...
    constructor Create(AAD: JAdView);
  public
...
constructor TMyAdViewListener.Create(AAD: JAdView);
begin
  try
    inherited Create;
    FAD := AAD;
...
procedure TMyAdViewListener.onAdClicked;
begin
...
procedure TMyAdViewListener.onAdFailedToLoad(adError: JLoadAdError);
begin
  FisShown := false;

  Log.D('Intersititial/Reward-FailedToLoad = ' + JStringToString(adError.toString));
  if (adError.getCode = 0) then
  ...
  else if (adError.getCode = 1) then
    RaiseException(FTestMode, 'AdUnitId inválida.' + sLineBreak +
      'ERROR_CODE_INVALID_REQUEST' + sLineBreak + 'Código de Erro: ' +
      adError.getCode.ToString)
  else if (adError.getCode = 2) then
...
procedure TMyAdViewListener.onAdImpression;
begin
...
{$IFDEF ANDROID}
    FAdMob := TJAdView.JavaClass.init(MainActivity);
