  ...
  try
    ...
    try
      ...
      if not FStorage.UploadObject(FBucketName, AFileName, LFileContent, False, LFileInformation, nil,
        TAmazonACLType.amzbaPublicReadWrite, LResponseInfo) then
...
begin
{$IFDEF ANDROID}
  LFilePath := TPath.GetTempPath + '/' + AFileName;
{$ELSE}
...
begin
  LResponseInfo := TCloudResponseInfo.Create;
  try
    FStorage.CreateBucket(ABucketName, TAmazonACLType.amzbaPublicReadWrite, Self.Configuration.Region, LResponseInfo);
